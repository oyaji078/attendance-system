...
class FakeRecognitionService:
    async def recognize(self, request, event_type: str, require_session: bool = False):
        assert event_type == "recognize"
        assert require_session is False
        assert len(request.frames) == 3
        return {"decision": "recognized", "reason": "multi_frame_confirm_passed", "confirmed_frames": 2, "device_code": request.device_code, "session_code": request.session_code, "person": {"person_id": str(uuid4()), "student_id": "ST-1001", "full_name": "Ada Lovelace", "template_id": str(uuid4())}, "confidence": 0.91, "top_candidates": []}
...
def test_recognition_endpoint_uses_three_frames() -> None:
    ...
    assert response.json()["decision"] == "recognized"
